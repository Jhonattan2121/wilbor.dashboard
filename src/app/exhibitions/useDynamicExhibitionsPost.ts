...
export function useDynamicExhibitionsPost(username: string) {
      ...
      try {
        ...
        if (found) {
          ...
          try {
            ...
            console.log('[Exhibitions] Tags extraídas:', postTags);
          } catch (e) {
            console.warn('Erro ao extrair tags:', e);
          }
          setTags(postTags);
        } else {
          setError('Nenhum post de exposições encontrado para este usuário.');
        }
      } catch {
